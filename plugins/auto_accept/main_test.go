...
func TestParseVerifyMsg(t *testing.T) {
	...
	if msg.SourceNickname != "管管" {
		t.Errorf("expected SourceNickname 管管, got %s", msg.SourceNickname)
	}
...
func TestBuildGreeting(t *testing.T) {
	...
	greeting := p.buildGreeting("小明", "管管")
	if !strings.Contains(greeting, "小明") {
		t.Errorf("greeting should contain nickname, got: %s", greeting)
	}
	if !strings.Contains(greeting, "我是管管介绍的肉丸。") {
		t.Errorf("greeting should mention '我是管管介绍的肉丸。', got: %s", greeting)
	}

	// Default greeting without sourceNickname (falls back to owner/default)
	greetingNoSource := p.buildGreeting("小红", "")
	if !strings.Contains(greetingNoSource, "我是管管介绍的肉丸。") {
		t.Errorf("greeting should mention '我是管管介绍的肉丸。', got: %s", greetingNoSource)
	}

	// Empty nickname
	greetingEmpty := p.buildGreeting("", "管管")
	if greetingEmpty != "你好，我是管管介绍的肉丸。" {
		t.Errorf("unexpected empty nickname greeting: %s", greetingEmpty)
	}

	// Custom greeting
	p.Config.CustomGreeting = "你好 {nickname}，{time_greeting}！我是 {bot}。"
	custom := p.buildGreeting("李雷", "管管")
	if !strings.Contains(custom, "李雷") || !strings.Contains(custom, "我是 肉丸。") {
